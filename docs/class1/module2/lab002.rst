...
Setting "noninteractive=true" option will perform a full unattended install if all the sample prompts below are provided and uncommented, otherwise the answers provided in pua_config.sh will be used as the defaults in a semi-interactive install method.
